...
test('accounts, wardrobe isolation, repeated generation and garment-reference image rendering',async()=>{
 ...
 const server=createApp({dataDir:dir,apiKey:'test-key',fetch:upstream});await new Promise(r=>server.listen(0,'127.0.0.1',r));const base='http://127.0.0.1:'+server.address().port;let cookie='';
 ...
 const two=await call('/api/generate-outfits',{});assert.notEqual(one.data.outfits[0].name,two.data.outfits[0].name);
